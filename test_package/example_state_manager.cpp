...
json
toServers(std::list<nuraft::ptr<nuraft::srv_config>> const& server_list) {
...
void
simple_state_mgr::save_config(const nuraft::cluster_config& config) {
...
void
simple_state_mgr::save_state(const nuraft::srv_state& state) {
   auto const state_file = fmt::format(FMT_STRING("s{}.store/state.json"), _srv_id);
   auto json_obj = json{
       { "term", state.get_term() },
