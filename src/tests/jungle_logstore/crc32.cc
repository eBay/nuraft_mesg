...
/**
 * copied from ForestDB project by Jung-Sang Ahn in 2018.
 */

/**
 * wrapped by Jung-Sang Ahn in 2019.
 */
...
uint32_t Crc32::get(const void* data, size_t len, uint32_t seed) {
    uint32_t* cur = (uint32_t*)data;
    uint32_t crc = ~seed;

    while (len >= 8) {
#if 0
        // Calculation for Big-endian platform.

        uint32_t one = *cur++ ^ bitswap32(crc);
        uint32_t two = *cur++;
        crc = crc_lookup[7][(one>>24) & 0xFF] ^
...
#else
        // Calculation for Little-endian platform.
...
uint32_t Crc32::get(const SEBuf& buf, uint32_t seed) { return get(buf.buf, buf.len, seed); }

uint32_t Crc32::get(const std::string& str, uint32_t seed) { return get(str.data(), str.size(), seed); }
