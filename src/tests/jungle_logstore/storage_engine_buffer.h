...
struct SEBuf {
     ...
     */
    SEBuf(size_t _len, void* _buf) : len(_len), buf(_buf) {}
    ...
    /**
     * Compare two given buffers, in a lexicographical order.
     *
     * @return -1 if `ll` <  `rr`.
     *          0 if `ll` == `rr`.
     *          1 if `ll` >  `rr`.
     */
    static int cmp(const SEBuf& ll, const SEBuf& rr) {
        int rc = memcmp(ll.buf, rr.buf, std::min(ll.len, rr.len));
        if (rc == 0) rc = _SCI(ll.len) - _SCI(rr.len);
        return rc;
    }
