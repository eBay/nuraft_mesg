...
struct jungle_log_store::log_cache {
    ...
    bool gets(ulong start, ulong end, int64_t size_hint, ptr< std::vector< ptr< log_entry > > >& vector_out) {
        int64_t cur_size = 0;
...
ssize_t jungle_log_store::getCompMaxSize(jungle::DB* db, const jungle::Record& rec) {
...
ssize_t jungle_log_store::compress(jungle::DB* db, const jungle::Record& src, jungle::SizedBuf& dst) {
    ...
    ssize_t comp_size =
        LZ4_compress_default((char*)src.kv.value.data, (char*)dst.data + 2, src.kv.value.size, dst.size - 2);
    return comp_size + 2;
}

ssize_t jungle_log_store::decompress(jungle::DB* db, const jungle::SizedBuf& src, jungle::SizedBuf& dst) {
    return LZ4_decompress_safe((char*)src.data + 2, (char*)dst.data, src.size - 2, dst.size);
}
...
jungle_log_store::jungle_log_store(const std::string& log_dir, const jungle_log_store::Options& opt) :
        logDir(log_dir),
    ...
    db_config.compOpt.cbGetMaxSize =
        std::bind(&jungle_log_store::getCompMaxSize, this, std::placeholders::_1, std::placeholders::_2);
    db_config.compOpt.cbCompress = std::bind(&jungle_log_store::compress, this, std::placeholders::_1,
                                             std::placeholders::_2, std::placeholders::_3);
    db_config.compOpt.cbDecompress = std::bind(&jungle_log_store::decompress, this, std::placeholders::_1,
                                               std::placeholders::_2, std::placeholders::_3);
...
ptr< std::vector< ptr< log_entry > > > jungle_log_store::log_entries_ext(ulong start, ulong end, int64_t size_hint) {
...
void jungle_log_store::shutdown() { jungle::DB::shutdown(); }
