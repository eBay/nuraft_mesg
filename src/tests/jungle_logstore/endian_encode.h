...
#include <algorithm>
#include <cstdint>
// Copied from Couchbase ForestDB project, and modified by Jung-Sang Ahn.
...
#define reverse_order_n(v, n)                                                                                          \
    {                                                                                                                  \
        auto *first = reinterpret_cast< uint8_t* >(v), *last = reinterpret_cast< uint8_t* >(v) + n;                    \
...
#define _enc_n(v, n) reverse_order_n(v, n)
#define _dec_n(v, n) reverse_order_n(v, n)
...
#ifdef __ENDIAN_SAFE
#define _enc(v)                                                                                                        \
    ((sizeof(v) == 8) ? (_enc64(v)) : ((sizeof(v) == 4) ? (_enc32(v)) : ((sizeof(v) == 2) ? (_enc16(v)) : (v))))
#define _dec(v)                                                                                                        \
    ((sizeof(v) == 8) ? (_dec64(v)) : ((sizeof(v) == 4) ? (_dec32(v)) : ((sizeof(v) == 2) ? (_dec16(v)) : (v))))
#else
#define _enc(v) (v)
#define _dec(v) (v)
#endif
