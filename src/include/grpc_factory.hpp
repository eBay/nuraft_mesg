...
class grpc_factory :
  public nuraft::rpc_client_factory,
  public std::enable_shared_from_this<grpc_factory>
{
 ...
 public:
    ...
    virtual
    std::error_condition
    create_client(const std::string &client, nuraft::ptr<nuraft::rpc_client>&) = 0;

    virtual
    std::error_condition
    reinit_client(nuraft::ptr<nuraft::rpc_client>&) = 0;

    // Construct and send an AddServer message to the cluster
    std::future<bool>
    add_server(uint32_t const srv_id, std::string const& srv_addr, nuraft::srv_config const& dest_cfg);

    // Send a client request to the cluster
    std::future<bool>
    client_request(shared<nuraft::buffer> buf, nuraft::srv_config const& dest_cfg);

    // Construct and send a RemoveServer message to the cluster
    std::future<bool>
    rem_server(uint32_t const srv_id, nuraft::srv_config const& dest_cfg);
};
