...
public:
    explicit group_metrics(group_id_t const& group_name) :
            sisl::MetricsGroupWrapper("RAFTGroup", to_string(group_name).c_str()) {
...
struct grpc_server_wrapper {
    explicit grpc_server_wrapper(group_id_t const& group_name);
...
public:
    ...
    NullAsyncResult add_srv(group_id_t const& group_name, nuraft::srv_config const& cfg);
    NullAsyncResult append_entries(group_id_t const& group_name,
                                   std::vector< nuraft::ptr< nuraft::buffer > > const& logs);
    NullAsyncResult rm_srv(group_id_t const& group_name, int const member_id);

    bool request_leadership(group_id_t const& group_name);
    ...
    nuraft::cmd_result_code createRaftGroup(int const srv_id, group_id_t const& group_name,
                                            group_type_t const& group_type) {
        return joinRaftGroup(srv_id, group_name, group_type);
    }

    nuraft::cmd_result_code joinRaftGroup(int32_t srv_id, group_id_t const& group_name, group_type_t const&);

    void partRaftGroup(group_id_t const& group_name);
